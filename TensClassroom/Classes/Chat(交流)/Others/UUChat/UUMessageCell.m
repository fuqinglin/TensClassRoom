...
- (void)btnContentClick{
    ...
    if (self.messageFrame.message.type == UUMessageTypeVoice) {
        if(!contentVoiceIsPlaying){
            ...
            audio.delegate = self;
            [audio playSongWithUrl:videoURL];
        }else{
...
- (void)setMessageFrame:(UUMessageFrame *)messageFrame{
    ...
    UIImage *normal;
    if (message.from == UUMessageFromMe) {
        normal = [UIImage imageNamed:@"chat_right"];
        normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(20, 20, 10, 10) resizingMode:UIImageResizingModeStretch];
    }
    else{
        normal = [UIImage imageNamed:@"chat_left"];
        normal = [normal resizableImageWithCapInsets:UIEdgeInsetsMake(20, 10, 10, 20) resizingMode:UIImageResizingModeStretch];
    }
    ...
    switch (message.type) {
        ...
        case UUMessageTypePicture:
        {
            ...
            NSURL *url = [NSURL URLWithString:message.pictureURL];
            UIImage *plaImage = [UIImage imageNamed:@"加载中"];
            [self.btnContent.backImageView setImageWithURL:url placeholderImage:plaImage];
        ...
        case UUMessageTypeVoice:
        {
            ...
            self.btnContent.second.text = [NSString stringWithFormat:@"%@ \"",message.strVoiceTime];
